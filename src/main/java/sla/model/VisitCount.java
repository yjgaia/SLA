...
public class VisitCount {
	...
	public static List<UserInfoWithCount> getCountRecordByUser(long id, int startPeriod, int endPeriod){
		String query = "SELECT c.*,u.* FROM  (SELECT user_id,SUM(cnt) cnt FROM ( SELECT a.*,(SELECT COALESCE(SUM(visit_count),0) FROM visit_count WHERE encoded_key_id=a.id  AND (time_period BETWEEN 2012103002 AND 2012103003)) AS cnt FROM short_url a WHERE head_id=5 ORDER BY cnt DESC ) count_by_user GROUP BY user_id ORDER BY cnt DESC) c , user_info u WHERE c.user_id=u.id ORDER BY cnt DESC";
